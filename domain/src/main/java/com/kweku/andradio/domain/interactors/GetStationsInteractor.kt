...
class GetStationsInteractor (private val repositoryInterface: RepositoryInterface,
    ...
    private val coroutineScope = CoroutineScope(Dispatchers.IO + parentJob )
     ...
     override fun sendAllStationsToUI() = coroutineScope.launch{
        allStationsOutput
            .updateAllStationsLiveData(getAllStationsAsync()
                .await())
    }


    private fun getPlayableStationAsync(stationId: String):Deferred<StationClickCount> = coroutineScope.async {
        return@async repositoryInterface.sendStationClick(stationId)
    }

    override fun sendPlayableStationToPresentation(station: Station):Job = coroutineScope.launch {
        playOutput.getStationToPlay(
            getPlayableStationAsync(station.stationUuid).await(),
            station
        )
    }
